import 'dart:io' show Platform, File, Process, ProcessStartMode;
...
// ProductsPage, _ProductCard, CreateProductPage, _GroupEditor, _OptionEditor,
// OrderWizard, _GroupStep, _Summary, CartPage, OrdersPage...
...
// --- ESC/POS byte üretici (IP/Windows ortak) ---
List<int> _buildEscPos(SavedOrder o) {
  ...
  for (int i = 0; i < o.lines.length; i++) {
    final l = o.lines[i];
    writeCp1252(
        _rightLine('Item ${i + 1}: ${l.product.name}', _money(l.total)) + '\n');
    for (final g in l.product.groups) {
  ...
  size(1);
  writeCp1252(
      _rightLine('TOTAL', '€${o.total.toStringAsFixed(2).replaceAll('.', ',')}') +
          '\n');
  size(0);
  boldOff();
...
// --- Ortak yazdırma fonksiyonu ---
// Windows: spooler (varsayılan yazıcı), Diğerleri: IP/RAW 9100
Future<void> printOrder(SavedOrder o) async {
  ...
  if (Platform.isWindows) {
    ...
    if (printer == null || printer.isEmpty) {
      throw Exception(
          'Varsayılan yazıcı bulunamadı. Windows ayarlarından bir yazıcı seçin.');
    }
    writeRawToPrinterWindows(printer, data);
    return;
  }

  // IP üzerinden yazdırma (Android/macOS/Linux için)
